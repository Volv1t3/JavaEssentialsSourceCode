...
public class BusinessTask extends Task implements Comparable<BusinessTask>{

    private static Integer BUSINESS_TASK_COUNTER = 0;
    /*! Parametros de BusinessTask*/
    private String businessTaskAsignee;

    /*! Implementaciones de la Clase BusinessTask*/

    public void setBusinessTaskAsignee(String businessTaskAsignee) throws InvalidTaskParameter{
        if (!businessTaskAsignee.isBlank()){
            this.businessTaskAsignee = businessTaskAsignee;
        }
        else {
            throw new InvalidTaskParameter("El nombre del asignado no puede estar vacio",
                    new Throwable("businessTaskAsignee = \" + businessTaskAsignee + \""));
        }
    }
    public String getBusinessTaskAsignee() {
        return businessTaskAsignee;
    }


    /*! Constructores de la clase*/
    public BusinessTask(Integer task_ID, String task_Description, QuickTimeStamps task_DueDate, String businessTaskAsignee, String businessName) throws InvalidTaskParameter{
        super(0, task_Description, task_DueDate);
        this.set_task_Notes(new BusinessNotes(businessName));
        try{
            this.set_task_ID(validateTaskIDInput(task_ID));
            this.setBusinessTaskAsignee(businessTaskAsignee);
        } catch (InvalidTaskParameter e) {
            throw new RuntimeException(e);
        }
    }

    public BusinessTask(Integer task_ID, String task_Description, QuickTimeStamps task_DueDate, String businessName){
        super(0, task_Description, task_DueDate);
        this.set_task_Notes(new BusinessNotes(businessName));
        try{
            this.set_task_ID(validateTaskIDInput(task_ID));
    ...
    public BusinessTask(){
        super(BUSINESS_TASK_COUNTER + 1, "", QuickTimeStamps.OCHO_AM);
        this.set_task_Notes(new BusinessNotes("undefined"));
    }
    ...
    public void set_task_Notes(ListableNotes taskNotes) {
        try{
            this._task_Notes = (BusinessNotes) taskNotes;
        }catch (ClassCastException classCastException){
            throw new InvalidTaskParameter("El parametro ingresado en el metodo set_task_Notes es incorrecto",
                    new Throwable("Parametro no es de tipo BusinessNotes").fillInStackTrace());
        }
    }

    @Override
    public ListableNotes get_task_Notes() {
    ...
    public Integer validateTaskIDInput(Integer task_ID) {
        if (task_ID > 0) {
            if (!Objects.equals(task_ID - 1, BUSINESS_TASK_COUNTER)) {
                throw new InvalidTaskParameter("El parametro ingresado en el metodo set_task_ID es incorrecto",
                        new Throwable("Parametro no sige secuencia fija de BUSINESS_TASK_COUNTER + 1, para la instancia" +
                                " que se intento crear").fillInStackTrace());
            } else {
                BUSINESS_TASK_COUNTER++;
                return task_ID;
            }
        } else {
            throw new InvalidTaskParameter("El parametro ingresado en el metodo set_task_ID es incorrecto",
    ...
    public String toString() {
        StringBuilder businessNoteAsString = new StringBuilder(super.toString());
        businessNoteAsString.delete(super.toString().length() -2, super.toString().length() -1);
        //! Anadimos el nuevo contenido generado por esta clase
        businessNoteAsString.append(", businessTaskAsignee='").append(businessTaskAsignee).append('\'');
        businessNoteAsString.append("}");
        return businessNoteAsString.toString();
    }
    ...
    public boolean equals(Object obj) {
        if (obj == null) {
            ...
            try{
                BusinessTask task = (BusinessTask) obj;
     ...
     */
    @Override
    public int compareTo(BusinessTask o) {
